const ajax = (options) => {
    ...
    if (defaultObj.type === 'GET') {
        ...
        if (defaultObj.header['Content-Type'] == 'application/x-www-form-urlencoded') {
            ...
            console.log('request is POST, header is: ' + defaultObj.header['Content-Type']);

            // TODO 请求头设置为application/x-www-form-urlencoded 格式send()中传递必须使用该格式
            // post请求通过send()传递参数
            console.log('defaultObj.params: ', defaultObj.params);
            xhr.send(defaultObj.params);
        } else if (defaultObj.header['Content-Type'] == 'application/json') {
